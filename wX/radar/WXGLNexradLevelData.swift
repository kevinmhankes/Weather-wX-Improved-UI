...
final class WXGLNexradLevelData {
    ...
    private var radarBuffers: ObjectOglRadarBuffers?
    var index = "0"

    convenience init(_ product: String, _ radarBuffers: ObjectOglRadarBuffers, _ index: String) {
        self.init()
    ...
    func decocodeAndPlotNexradLevel3FourBit() {
        ...
        if dis.capacity > 0 {
            ...
            dis.skipBytes(56)
            dis.skipBytes(32)
            numberOfRangeBins = Int(UtilityWXOGLPerfL3FourBit.decode4Bit(radarBuffers!))
            binSize = WXGLNexrad.getBinSize(productCode)
            numberOfRadials = 360
        }
    }
    ...
    func decocodeAndPlotNexradL2() {
        ...
        UtilityWXOGLPerfL2.level2Decompress(radarBuffers!)
        Level2.decode(radarBuffers!, days, msecs)
        writeTimeL2()
        binSize = WXGLNexrad.getBinSize(productCode)
        binWord.position = 0
    ...
    func writeTimeL2() {
        ...
        editor.putString("WX_RADAR_CURRENT_INFO", radarInfoFinal)
    }
