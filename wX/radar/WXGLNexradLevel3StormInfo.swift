...
class WXGLNexradLevel3StormInfo {
            ...
            if (posnNumbers.count == motNumbers.count) && posnNumbers.count > 1 {
                stride(from: 0, to: posnNumbers.count - 2, by: 2).forEach { index in
                    ...
                    ec = ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, start,
                                                              Double(degree), Double(nm) * 1852.0, bearing)
    ...
    static func drawTickMarks(_ startPoint: LatLon, _  ecc: ExternalGeodeticCalculator,
                              _ pn: ProjectionNumbers, _ ecArr: ExternalGlobalCoordinates,
