    ...
    static func decocodeAndPlot(_ pn: ProjectionNumbers, isGust: Bool) -> [Double] {
        ...
        arrWb.forEach { windBarb in
            ...
            if length>4 {
                ...
                ec =  ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, start, degree, nm * nmScaleFactor * barbLengthScaleFactor, bearing)
                ...
                if above50 {
                    // initial angled line
                    ec = ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, end, degree2,
                                                              barbOffset + startLength + Double(index) * arrowSpacing * nmScaleFactor * barbLengthScaleFactor, bearing)
                    ...
                    ec = ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, end, degree2,
                                                              barbOffset + startLength + -1.0 * arrowSpacing * nmScaleFactor * barbLengthScaleFactor, bearing)
                    ...
                    ec = ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, end, degree2,
                                                              barbOffset + startLength
                                                                + Double(index) * arrowSpacing * nmScaleFactor * barbLengthScaleFactor, bearing)
                    stormList += drawLine(ec, ecc, pn, start, degree2 - 180.0, startLength + 0.5 * arrowLength * nmScaleFactor, bearing)
                    index += 1
                }
                (index..<barbCount).forEach { _ in
                    ec = ecc.calculateEndingGlobalCoordinates(ExternalEllipsoid.WGS84, end, degree2,
                                                              barbOffset + startLength
                                                                + Double(index) * arrowSpacing * nmScaleFactor * barbLengthScaleFactor, bearing)
                    stormList += drawLine(ec, ecc, pn, start, degree2 - arrowBend * 2.0, startLength + arrowLength * nmScaleFactor, bearing)
                    index += 1
                }
