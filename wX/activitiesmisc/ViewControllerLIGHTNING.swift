...
class ViewControllerLIGHTNING: UIwXViewController {
    ...
    @objc func prodClicked() {
        ...
        UtilityLightning.sectorList.enumerated().forEach { index, rid in
            alert.addAction(UIAlertAction(title: rid, style: .default, handler: {_ in self.sectorChanged(index)}))
        }
        alert.finish()
    }
    ...
    @objc func timeClicked() {
        ...
        UtilityLightning.timeList.enumerated().forEach { index, rid in
            alert.addAction(UIAlertAction(title: rid, style: .default, handler: {_ in self.timeChanged(index)}))
        }
        alert.finish()
    }
